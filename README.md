...
- “本组最佳”个人偏好与改选历史
- VLM 对比原图/候选图 → 本地 LLM 优化 Prompt/参数 → 同 seed 改进
- 识图、生图、诊断和更正任务的等待进度层与真实耗时显示
- 手机局域网 UI
...
```text
当前设置   = 保留当前 LoRA、workflow 分辨率和双 sampler
平衡重建   = 关闭 LoRA + 单 sampler + 35 steps + CFG 6 + 原图纵横比
...
候选图上的“设为本组最佳”会保存当前偏好，同时保留每次改选历史。点击
“AI 对比原图 → 同 seed 改进 1 张”后，Studio 会把原图与该候选组成带标签的
本地对比图，先由 VLM 诊断内容、风格、构图与互动关系，再由 `qwen3:8b` 结合
实际 Prompt、负面词、生成参数、评分和本组最佳记录给出受约束的修改方案。
新候选沿用原 seed；Prompt、参数建议、视觉报告和父 Generation 都写入 SQLite，
不会改写 Analyzer Final Prompt，也不会把重建 Prompt 记作 Original Prompt。
...
2. 用累积的“本组最佳”与 AI 复盘记录实现跨图片相似案例检索
